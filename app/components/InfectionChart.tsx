...
export default function InfectionChart() {
  ...
  // Definición de datasets para cada variable
  const datasets = {
    defunciones: {
  ...
  // Definición de la data para el gráfico
  const chartData: ChartData<"line", number[], number> = {
    labels: years,
    datasets: [datasets[selectedDataset]]
  };

  // Opciones del gráfico tipadas como ChartOptions<"line">
  const chartOptions: ChartOptions<"line"> = {
    ...
    plugins: {
      legend: { 
        display: true, 
        position: "bottom" as const, // Se fuerza el literal "bottom"
    ...
    scales: {
      ...
      y: { 
        title: { 
          display: true, 
          text: selectedDataset === "defunciones" 
                ? "Defunciones Totales" 
  ...
  return (
    <div className="p-6 border-4 border-gray-700 dark:border-gray-300 rounded-lg">
      <h2 className="text-2xl font-semibold text-center mb-4">
        Mortalidad por Enfermedades Infecciosas No Transmisibles
      </h2>
