...
const accidentData: Record<string, { Total: number }> = {
    "Pinar del Rio": { Total: 31 }, "Artemisa": { Total: 58 }, "La Habana": { Total: 178 },
    "Mayabeque": { Total: 27 }, "Matanzas": { Total: 79 }, "Villa Clara": { Total: 62 },
    "Cienfuegos": { Total: 33 }, "Sancti Spiritus": { Total: 37 }, "Ciego de Avila": { Total: 34 },
    "Camaguey": { Total: 51 }, "Las Tunas": { Total: 34 }, "Holguin": { Total: 68 },
    "Granma": { Total: 49 }, "Santiago de Cuba": { Total: 70 }, "Guantanamo": { Total: 41 },
...
const interpolateColor = (minColor: string, maxColor: string, factor: number) => {
    ...
    const resultRGB = minRGB.map((min, i) => Math.round(min + (maxRGB[i] - min) * factor));
...
const provinceColors = [
    "#FF0000", "#0000FF", "#00FF00", "#FFA500", "#800080", "#008080",
    "#FFC0CB", "#FFD700", "#A52A2A", "#00FFFF", "#7CFC00", "#DC143C"
];
        ...
        <div className="w-full flex flex-col items-center">
            {geoData ? (
                <MapContainer
                    ...
                    style={{ height: "400px", width: "100%", backgroundColor: "turquoise" }}
                    dragging={false}
                    zoomControl={false}
                ...
                >
                    <GeoJSON
                        data={geoData}
                        style={(feature) => {
                            const province = feature.properties.province.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
                            return {
                        ...
                        onEachFeature={(feature, layer) => {
                            ...
                            layer.bindTooltip(`
                                <div style="text-align: center;">
                                    ...
                                    <span style="color: red;">Accidentes Totales 2023: ${accidentData[province]?.Total || 0}</span>
                                </div>
                            `, { sticky: true });
            ...
            {selectedProvinces.length > 0 && (
                <div className="w-full mt-8">
                    <h2 className="text-center text-lg font-bold">Mortalidad (2005-2022)</h2>
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={years.map((year, i) => {
                            const dataPoint: any = { year };
                            selectedProvinces.forEach((province) => {
                                dataPoint[province] = accidentHistory[province]?.[i] ?? 0;
                            });
                            return dataPoint;
                        })}>
                            <XAxis dataKey="year" />
                            ...
                            {selectedProvinces.map((province, index) => (
                                <Line key={province} type="monotone" dataKey={province} stroke={provinceColors[index % provinceColors.length]} />
                            ))}
                        </LineChart>
                    </ResponsiveContainer>
                </div>
